...
$_PROGRAM_TITLE		 = "UPNP-MULTICAST NETGEAR ReadyNAS Perl Communications Library";
	...
	if (getQueryUnitCount()) {
		...
		for ( split /\|/, $address_list ) {
			...
			if ($_ ne "") {
				...
				writeLog($_OUTPUT_VERBOSE,"loadReadyNASData","Opening I/O Monitor");

				$iosock = createIOMonitor();

				writeLog($_OUTPUT_VERBOSE,"loadReadyNASData","Adding UDP Address");

				addUDPAddress(getBroadcastAddress());

				writeLog($_OUTPUT_VERBOSE,"loadReadyNASData","Adding Socket to I/O Monitor");

				addSocketToMonitor($sock);

				writeLog($_OUTPUT_VERBOSE,"loadReadyNASData","Sending Packet");

				sendUDPPacket($sock,$packet,getDestinationAddress(),getDstPort());
	
				writeLog($_OUTPUT_VERBOSE,"loadReadyNASData","Sent " . getBytesSent() . " bytes to " . getBroadcastAddress() . ":" . getDstPort());
...
sub parseReadyNASData{
	...
	if ($invalid_packet) {
		...
		$unit{$macindex}{system}{uptime}{0} = "N/A";
		$unit{$macindex}{system}{memory}{0} = "N/A";
		$unit{$macindex}{system}{processes}{0} = "N/A";
		$unit{$macindex}{system}{uname}{0} = "N/A";
...
#-----------------------------
sub createIOMonitor{
	writeLog($_OUTPUT_DEBUG,"createIOMonitor","Creating I/O Monitor");
	$iosock = IO::Select->new();
	return $iosock;
}

#-----------------------------
sub addSocketToMonitor{
	my $socket = shift;
	writeLog($_OUTPUT_DEBUG,"addSocketToMonitor","Adding $socket I/O Monitor");
	return $iosock->add($socket);	
}
...
#-----------------------------
sub createUDPSocket{
	writeLog($_OUTPUT_DEBUG,"createUDPSocket","Creating UDP Multicast Socket");
	$sock = IO::Socket::Multicast->new(Proto     => 'udp',
					   Broadcast => 1);
	return $sock;
}
...
sub addUDPAddress{
	my $address = shift;
	writeLog($_OUTPUT_DEBUG,"addUDPAddress","Adding $address to UDP Multicast");
	return $sock->mcast_add($address);
}
...
sub sendUDPPacket{
	...
	return $sock->mcast_send($packet,"$address:$port");
}
...
sub receiveUDPPackets{
	...
	while(@ready = $iosock->can_read(getSocketTimeout())) {
		foreach $shandle (@ready) {
        		if ($shandle == $sock) {
...
sub checkForError{
	my $socket = shift;
	my $sockerr = IO::Select->new();	
	my $shandle = "";
	my $retcode = 0;
	my @errors;

	writeLog($_OUTPUT_DEBUG,"checkForError","Start");

	writeLog($_OUTPUT_DEBUG,"checkForError","Adding Error Check to I/O Monitor");

	$sockerr->add($sock);

	writeLog($_OUTPUT_DEBUG,"checkForError","Monitoring for Errors for handle=$sock");

	@errors = $sockerr->has_error(getSocketErrorTimeout());

	writeLog($_OUTPUT_DEBUG,"checkForError","Examining Data");

	foreach $shandle (@errors) {
		if ($shandle == $socket) {
			writeLog($_OUTPUT_DEBUG,"checkForError","Socket Error Found");
			$retcode = 1;
		} 
	}

	writeLog($_OUTPUT_DEBUG,"checkForError","Exit($retcode)");
...
sub readSocket{
	...
	writeLog($_OUTPUT_DEBUG,"readSocket","Checking For Error");

	$error = checkForError($sock);

	if ($error) {
		writeLog($_OUTPUT_DEBUG,"readSocket","Socket Error or EOF");
		writeLog($_OUTPUT_DEBUG,"readSocket","Exit(1)");
		return ("0.0.0.0","ERROR");
	}else{
		my $peer = recv($sock,$data,getSocketBufferSize(),getSocketFlags());
		if ($peer ne "") {	
